...
if( $_POST['submit'] ){
            ...
            if( $filetype['type'] == 'image/jpeg' || $filetype['type'] == 'image/png' ){
                ...
                if( $fsize < $fsize_limit ){
                    if( move_uploaded_file( $_FILES['user_photo']['tmp_name'], $destination_file ) ){
                        ...
                        $errors[] = '<span style="color:#FF0000;">Error upload';
                    }
                } else {
                    $errors[] = '<span style="color:#FF0000;">Size of image must be small than ' . $fsize_limit . ' KByte.';
                }
            } else {
                $errors[] = '<span style="color:#FF0000;">Extension of image must be jpg and png.';
            }
        }
    }
    ...
    if ($_POST['companyname'])
        ...
        $errors[] = '<span style="color:#FF0000;">Please insert company name';
   ...
   // $info['state'] = $_POST['comp_state'];
	...
	if (empty($_POST["comp_city"])) {
    $errors[] = "City cannot be left blank";
	}
	
	if (empty($_POST['comp_zip_code'])) {
    $errors[] = "Zip Code cannot be left blank";
	}
	
	if (empty($_POST['address1'])) {
    $errors[] = "Address line 1 cannot be left blank";
	}
   ...
   if($_POST['companyurl']!=''){
	...
	$info['companyurl'] =''; 
	$errors[] = '<span style="color:#FF0000;">Not A Valid Company Url.'; 
	}else
	{
		if (strpos($_POST['companyurl'],'http://') === false) {
          			
			$info['companyurl']='http://'.$_POST['companyurl'];
   ...
   if($_POST['twitter']!=''){
	...
	$info['twitter'] =''; 
	$errors[] = '<span style="color:#FF0000;">Not A Valid Twitter Url.'; 
	}else
	{
		if (strpos($_POST['twitter'],'http://') === false) {
          			
			$info['twitter']='http://'.$_POST['twitter'];
	...
	$info['linkedin'] =''; 
	$errors[] = '<span style="color:#FF0000;">Not A Valid Linkedin Url.'; 
	}else
	{
		if (strpos($_POST['linkedin'],'http://') === false) {
          			
			$info['linkedin']='http://'.$_POST['linkedin'];
	...
	$info['googleplus'] =''; 
	$errors[] = '<span style="color:#FF0000;">Not A Valid Googleplus Url.'; 
	}else
	{
		if (strpos($_POST['googleplus'],'http://') === false) {
          			
			$info['googleplus']='http://'.$_POST['googleplus'];
	...
	$info['facebook'] =''; 
	$errors[] = '<span style="color:#FF0000;">Not A Valid Facebook Url.'; 
	}else
	{
		if (strpos($_POST['facebook'],'http://') === false) {
          			
			$info['facebook']='http://'.$_POST['facebook'];
	...
	$errors[] = '<span style="color:#FF0000;">Please enter First name';
    }      
    else update_user_meta($current_user->ID, 'first_name', $_POST['first_name']);
}

                              
        if(isset($_POST['last_name'])){
	if(empty($_POST['last_name'])){
        $errors[] = '<span style="color:#FF0000;">Please enter Last name';
         }
           else update_user_meta($current_user->ID, 'last_name', $_POST['last_name']);
 }         
    ...
    if (preg_match('/^[0-9]+$/', $_POST['phoneprim'])) {
		...
		$errors[] = '<span style="color:#FF0000;">Enter a valid Phone Number.'; 
	}
    ...
    if ($_POST['user_email']) {
        if (!is_email($_POST['user_email'])) {
            $errors[] = '<span style="color:#FF0000;">Not A Valid Email address.';
        } else {
            if ($current_user->user_email != $_POST['user_email']) {
                if (email_exists($_POST['user_email'])) {
                    $error = '<span style="color:#FF0000;">Not A Valid Email address.';
                } else {
...
.span12 #map-canvas{height:0 !important;}
...
jQuery( document ).ready(function() {		
	jQuery('.side2').css('min-height', (jQuery('.side1').height()+31)+'px');
});
</script>
            ...
            <div id="jquery-live-validation-edit-profile">
             <!-- /* basic container starts */ -->
                ...
                <form method="post" action="" name="profile-form" enctype="multipart/form-data">
                    ...
                    <!-- /* Company image holder starts  */ -->
                    <div class="company-image-holder">
					...
					<div class="">
                                ...
                                if ($current_company->logo_file_nm) {
									...
									else
									{
										...
										$img_url='http://www.itlocator.com/wp-content/themes/itlocation/images/no-image1.png';
									}
                                    ?>
                                    <div class="fileupload fileupload-exists" data-provides="fileupload"><input type="hidden" value="" name="">


<div class="fileupload-preview fileupload-exists thumbnail max-width-200 line-height-20">
	<img src="<?php echo $img_url; ?>" class="" id="uploadPreview1"></div>
<div>
		<input id="uploadImage1" type="file" name="comp_logo" onchange="PreviewImage(1);" />
	</div>
        ...
        oFReader.onload = function (oFREvent) {
            document.getElementById("uploadPreview"+no).src = oFREvent.target.result;
                    ...
                    <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" id="companyname" name="companyname" value="<?php echo $current_company_a['companyname'] ?>" placeholder="<?php _e('Company Name', 'twentyten'); ?>" maxlength="100">
                                </div>
                    ...
                    <div class="edit-container">
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6">
                            	<div class="edit-holder side1">
                                
                                 <h4 class="box-title"><i class="fa fa-briefcase"></i> <?php _e('Company Details', 'twentyten') ?></h4>
                        
                                
                                
                                <div class="form-group">
                                    <label class="control-label" for="address1">Headquarters Address Line 1 <span class="imp_star_mark">*</span></label>
                                    <input class="form-control" type="text" id="lo" name="address1" value="<?php echo $current_company_a['address1']; ?>" placeholder="Address Line 1">
                                </div>
                        <?php
                        ...
                        <div class="form-group" id="comp_state_grp">
                            <label class="control-label" for="comp_state"><?php _e('State/Province', 'twentyten') ?><span class="imp_star_mark">*</span></label>
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label class="control-label" for="comp_city"><?php _e('City', 'twentyten') ?><span class="imp_star_mark">*</span></label>
                                    <div class="loading" style="display: none"></div>
                                    <div id="comp_state_ctrl">
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                <label class="control-label" for="comp_zip_code"><?php _e('Zip Code/PC', 'twentyten'); ?><span class="imp_star_mark">*</span></label>
                        ...
                        <div class="form-group">
                        <label class="control-label" for="User Picture"><?php _e('User Picture', 'twentyten') ?></label>
                        <div class="profile-img-holder">
                    	<!-- /* Img and info */ -->
                        
                           <?php $user_photo_nm = get_user_meta($current_user->ID, 'user_photo_nm', true);
                            if ($user_photo_nm) {
                                $img_url = $upload_dir["baseurl"] . "/user_photos/" . $user_photo_nm;
									}
									else
									{
										//$img_url='http://placehold.it/150x150&text=User image';
										$img_url='http://www.itlocator.com/wp-content/themes/itlocation/images/image1.jpg';
									}
                                    ?>
                               <!-- <div class="user-img">
                                	<img src="<?php echo $img_url; ?>" id="uploadPreview2" class="img-responsive"/>
                                 <!--   <div class="controls">
                                ...
                                <!--</div>-->
                                <div class="user-img">
                                    <input type="file" name="user_photo" id="uploadImage2" onchange="PreviewImage(2);"/>
                                    <div id="preview">
                                        <img src="<?php echo $img_url; ?>" id="uploadPreview2" class="img-responsive"/>
                                    </div>
                                </div>
                            ...
                            <div class="media-body">
                               <!-- <h4 class="media-heading"><?php echo $current_user->user_login; ?></h4>-->
                               <!-- <p>Membership Status :<?php _e(S2MEMBER_CURRENT_USER_ACCESS_LABEL, 'twentyten') ?></p>-->
                            </div>
<!--<input type="file" name="user_photo" id="uploadImage2" onchange="PreviewImage(2);"/>-->
                        <!-- /* Img and info ends */ -->
                    </div>
			...
			if (get_option('itlocation_generals_contributions_edit_page')) {
           	...
           	$tmp_url = get_permalink($pid);
       		 ...
       		 ?>
			 <div class="row-fluid">
                            <h4 class="page-title-diff dMx"><?php _e('Content Contributions', 'twentyten') ?></h4>
				<a class="btn btn-sm btn-primary" href="<?php echo $tmp_url ?>"><?php _e('Add Content', 'twentyten') ?></a>
			</div>
                        </div>
                    </div>
                </form>
