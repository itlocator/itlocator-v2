...
*/
$errFlag=true;
if(empty($_REQUEST['claim_name']))
{
	$err="Name is Required";
	$errFlag=false;
}
if(empty($_REQUEST['claim_title']) && $errFlag==true)
{
	$err="Title is Required";
	$errFlag=false;
}
if(empty($_REQUEST['claim_to_email']) && $errFlag==true)
{
	$err="Email is Required";
	$errFlag=false;
}
if (!is_email($_REQUEST['claim_to_email']) && $errFlag==true) {
	  $err = 'Not A Valid Email address.';
	  $errFlag=false;
  } 
if(empty($_REQUEST['claim_phone']) && $errFlag==true)
{
	$err="Phone is Required";
	$errFlag=false;
}
if(preg_match('/^[a-zA-Z]+[a-zA-Z0-9._]+$/',$_REQUEST['claim_phone']) && $errFlag==true)
	{
	//invalid
...
//Check if no error found then send email
if($errFlag==true){
...
$email_content .= '<p>Email : ' . $_REQUEST['claim_to_email'] . '</p>';
...
$email_content .= '<p><a href="' . $approveclaim_url . '?userid=' . $_REQUEST['user_id'] . '&email=' . $_REQUEST['claim_to_email'] . '&company=' . $_REQUEST['claim_company'] . '">Approve Claim</a> | <a href="' . $denyclaim_url . '?userid=' . $_REQUEST['user_id'] . '&email=' . $_REQUEST['claim_to_email']. '&company=' . $_REQUEST['claim_company'] . '">Deny Claim</a></p>';

//$send_email_address = array( get_option('admin_email'), 'xue_developer@126.com' );
$send_email_address = array( get_option('admin_email'));
//echo send_mail( $send_email_address,$_REQUEST['claim_to_email'], $email_title, $email_content );
echo send_mail( $send_email_address,$_REQUEST['claim_to_email'], $email_title, $email_content );
}else{
	$errorelement='<div style="" class="alert alert-error span8 padding-only-bottom-5 padding-only-top-5 margin-only-top-10 margin-only-left-3 margin-only-bottom-0">'.$err.'</div>';
echo $errorelement;	
}
