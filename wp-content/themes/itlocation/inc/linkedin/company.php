...
try {
	...
	switch($_REQUEST[LINKEDIN::_GET_TYPE]) {
		case 'initiate':
			...
			//print_r($API_CONFIG['callbackUrl']);
...
/*
/*THIS CODE IS ADDED FOR SAME FUNCTIONALITY WHICH WORK FOR
*/					 
				...
				$linkedin_profile_email = json_decode($profile['linkedin']);
				
				 //echo "keywords=" .  $_REQUEST['company'] . "<br/>";
				// echo "<pre>" . print_r($response, TRUE) . "</pre>";
				 //echo "<pre>" . print_r($fallowedcompany, TRUE) . "</pre>";
				// exit;
				...
				if($response['success'] === TRUE) {
					...
					if( $linkedin_data->_total > 0 ) {
						foreach( $linkedin_data->values as $item ){
						// echo $item->name."==".$keywords;
						  //echo "<br/>";
						 if(strcmp($matchkey,$keywords)==0){
					...
					$exist_company="samelistsecond";
				}
		//==========================================================			 
					//header('Location: ' . RETURN_URL);
				} else {
					/* IF ANY USER TRY TO CLAIM SAME LISTING AGAIN  */
					$exist_company="samelistsecond";
					//echo "Access token retrieval failed:<br /><br />RESPONSE:<br /><br /><pre>" . print_r($response, TRUE) . "</pre><br /><br />LINKEDIN OBJ:<br /><br /><pre>" . print_r($OBJ_linkedin, TRUE) . "</pre>";
				}
				?>
<script type="text/javascript">
	alert("Your linkdin authentication process has been completed..proceed further for claim listing");
</script>
<?php
			}
			break;
		...
		default:
			...
			if($_SESSION['oauth']['linkedin']['authorized'] === TRUE) {
				...
				// $keywords = 'Apple';
			    $query    = '?sort=relevance&keywords=' . urlencode($keywords);
				//$response1 = $OBJ_linkedin->searchCompanies($query); 
				$response = $OBJ_linkedin->followedCompanies(); 
				$profile  = $OBJ_linkedin->profile('~/email-address');
				$linkedin_profile_email = json_decode($profile['linkedin']);
