...
if (!class_exists("shortcodesItlocation")):
    ...
    class shortcodesItlocation {
        ...
        function __construct() {
			...
			add_action('wp_ajax_upgradecoupon-itlocation', array(&$this, 'upgradecoupon'));
            add_action('wp_ajax_nopriv_upgradecoupon-itlocation', array(&$this, 'upgradecoupon'));
			...
			add_shortcode('upgradecoupon-itlocation', array(&$this, 'upgradecoupon'));
        ...
        function signup() {
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
				...
				$coupon_code = $_POST["coupon_code"];
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">You have already account.'
					));
					exit;
				}
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Company Name is empty.'
					));
					exit;
				}
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Please Enter a Valid Email Address'
					));
					exit;
				}
				...
				if( email_exists( $user_email ) ){
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">User Email is already exists.'
					));
					exit;
				}
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Please Enter a User Name'
					));
					exit;
				}
				...
				if( username_exists( $username ) ){
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">User Name already exists.'
					));
					exit;
				}
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Password is empty.'
					));
					exit;
				}
				...
				if( strlen( $user_password ) < 6 ){
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Password should have at least 6 letters.'
					));
					exit;
				}
                ...
                if( $resp->error ){
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Retype the security text.'
					));
					exit;
                }
				...
				if( gettype( $user_id ) != 'integer' ){
					...
					echo json_encode(array(
						'error' => true,
						'error_message' => '<span style="color:#FF0000;">Please check your user info.'
					));
					exit;
				}
				...
				$company_model->insert( $user_id, $company_name, $user_email, $source );
		...
		function inter_upgradecoupon_form($atts, $content) {
            ob_start();
		...
		/**
		* function Upgrade coupon code
		*/
		
		function upgradecoupon() {
            check_ajax_referer('upgradecoupon-itlocation', 'security');
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
