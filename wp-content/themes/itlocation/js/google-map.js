...
function reflesh_map_by_location( $icon_url, $root_val ){
    ...
    jQuery( '.locations' ).each(function( i ){
		...
		jQuery(".locations_lat" , jQuery($this).parent().parent()).val( results[0].geometry.location.lat() );

		jQuery(".locations_lng" , jQuery($this).parent().parent()).val( results[0].geometry.location.lng() );
...
/*function choose_item()
{
		...
		if( jQuery("#countries").val() == 'byaddress' ){
...
//	{
	...
	if( results[0].geometry.viewport ){
                   ...
                   //Before there is using "A instead of B"
					suffix_param = "&x1=" + pointNorthEast.k + "&y1=" + pointNorthEast.B + "&x2=" + pointSouthWest.k + "&y2=" + pointSouthWest.B +'&latitude=' + latitude + '&longitude=' + longitude;
				}		
					url_paramlist = suffix_param + '&lo=' + myaddress + '&action=new-by-address-search-map&ke=' + jQuery('#keywords').val() + jQuery('#se').serialize();
